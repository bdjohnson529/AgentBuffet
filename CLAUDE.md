...
1. **Research Mode**: Run **one** master script to pull all data, then save findings in `stocks/[TICKER]/research.md`.
   - **Data refresh:** From repo root run `python run.py`. This reads tickers from `stocks.txt` (one per line, optional $ prefix), runs every script in `scripts/` for each ticker, and writes news, financials, filings, insider, prices, estimates, and peers into `stocks/[TICKER]/`. A run summary is written to `reports/data_run_YYYY-MM-DD.md`.
